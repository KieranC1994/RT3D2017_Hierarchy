...
void Aeroplane::UpdateMatrices(void)
{
	...
	// [Skip this step first time through] Get the forward vector out of the world matrix and put it in m_vForwardVector
	 ...
	 m_vForwardVector = (m_mWorldMatrix.r[2]);					//pull the correct row from the world matrix to place into the forward vector
	...
	XMMATRIX m_mLocalProp = mRotX * mRotY * mRotZ * mTrans;
	...
	XMMATRIX m_mLocalTurret = mRotX * mRotY * mRotZ * mTrans;
	...
	XMMATRIX m_mLocalGun = mRotX * mRotY * mRotZ * mTrans;
	m_mGunWorldMatrix = m_mLocalGun * m_mTurretWorldMatrix;

	// Calculate m_mCamWorldMatrix for camera based on Euler rotation angles and position data.

	m_mCamWorldMatrix = XMMatrixTranslationFromVector(XMLoadFloat4(&m_v4CamOff));
	...
	if (m_bGunCam)
	{
		mPlaneCameraRot = mRotY * mTrans;
		m_mCamWorldMatrix = XMMatrixTranslationFromVector(XMLoadFloat4(&m_v4CamOff));
		m_mCamWorldMatrix = m_mCamWorldMatrix * m_mGunWorldMatrix;
	}
...
void Aeroplane::Update(bool bPlayerControl)
{
	...
	if(bPlayerControl)
	{
		...
		if (Application::s_pApp->IsKeyPressed('P') && (m_v4Rot.z > -20))
		{
			m_v4Rot.z --;
			m_v4Rot.y ++;
		}
